...
def test_get_assignments_in_graded_state_for_each_student(reset_assignment_state_2):
    """Test to get graded assignments for each student"""

    # Find all the assignments for student 1 and change its state to 'GRADED'
    submitted_assignments: Assignment = Assignment.filter(
    ...
    reset_assignment_state_2(2, 'DRAFT')
    reset_assignment_state_2(5, 'SUBMITTED')
...
def test_get_grade_A_assignments_for_teacher_with_max_grading(remove_assignments_from_id):
    """Test to get count of grade A assignments for teacher which has graded maximum assignments"""
    ...
    # Execute the SQL query and check if the count matches the created assignments plus the initial count
    sql_result = db.session.execute(text(sql)).fetchall()
    ...
    # Execute the SQL query again and check if the count matches the newly created assignments plus the initial count
    sql_result = db.session.execute(text(sql)).fetchall()
    assert grade_a_count_2 == sql_result[0][1]

    remove_assignments_from_id(7)
