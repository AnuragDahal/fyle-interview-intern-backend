...
#     # Create new assignments for student 1
#     for _ in range(5):
#         assignment = Assignment(
#             teacher_id=1,
#             student_id=1,
...
#     graded_assignments: Assignment = Assignment.query.filter(
...
#     sql_result = db.session.execute(text(sql)).fetchall()
#     expected_result = max(grade_a_count_1, grade_a_count_2)
#     assert expected_result == sql_result[0][1]
...
def test_get_assignments_in_graded_state_for_each_student():
    ...
    submitted_assignments: Assignment = Assignment.filter(
        Assignment.student_id == 1, Assignment.state == AssignmentStateEnum.GRADED).count()
